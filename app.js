...
function renderTypePeek() {
  const peekIndexes = [0, 1, 6, 15];
  $("#peekGrid").innerHTML = peekIndexes.map((index) => {
    return `
      <div class="peek-card">
        <div class="peek-art" style="background-position: ${spritePosition(index)}" aria-hidden="true"></div>
        <span>${typeNames[index]}</span>
      </div>
    `;
  }).join("");
}
